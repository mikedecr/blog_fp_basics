...
Functional framework would be different.
...
That new function defines a recipe for evaluating the functions $f$ and $g$ in sequence on some unknown data `x`, whever it happens to come across such data---we didn't pass any `x` yet.
...
You may have seen online flame wars about "strong typing".
...
Well...sorry!
Functional programming approaches like types because types provide structure to function composition at scale: functions can be composed if the output type of one function is the input type of the next function.
This lets us build really big, abstract structures out of function composition, maybe even structures that are so big and complicated that we struggle to keep track of it all in our minds.
But these structures are _virtually guaranteed to work_ as long as it is composed of functions with conforming input and output types.

Not coincidentally, the associativity of function composition is additionally helpful for API design.
If we have to build a big, abstract structure that, we can, at any point that we like, group some of the operations together if they form a coherent, reusable composition.
...
Let's apply this to the `english_n_unique` example.
...
\begin{align}
   ...
   \mathtt{length} &: \mathtt{.any} \mapsto \mathtt{integer} \\
...
We know that length takes `many` and returns its length, however that method is defined.
And `engligh::english` turns `many` objects into `english` type.
...
As long as I know the type behavior of each function, I can anticipate the type behavior of the compositions.


## Something bigger: means within groups

This is where we really start to see the difference between functional programming "trees" and more typical "sequences of imperative statements."
...
head(mtcars, 5)
```
...and calculate the mean of each variable as a new data frame.
...
2. define a function to apply this routine to groups of data
...
```
...which works when we call it with `mtcars`.
```{r}
means(mtcars)
```
...
Notice, this function takes two objects and returns one object.
In order to nicely compose it with other functions that take and return only one object, I want a way to reduce the arguments supplied.
This is a little weird, but I am going to do something that is evocative of a python tool called a `partial`, which takes a function `f` and returns a new function `f*`, which is just like `f` but with some of the arguments pre-filled.
...
But the function isn't evaluated; it is only created.
The user has to evaluate it later.

Here we use this tool to create a partial `as.character`, which we apply to some `x`.

```{r}
make_chars = partial_apply(as.character)
...
It certainly isn't as succinct as `dplyr::summarize`, but remember, functional programming hasn't been invented yet.
