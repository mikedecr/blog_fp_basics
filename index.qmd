...
1. Many intermediate R users have a _thing_ against for-loops because for-loops have a reputation for being slow in R,[^loops]
   ...
   This is referred to in these circles as "functional programming" because, rather than procedurally manipulating data explicitly, you are taking a pre-defined function and merely applies it to data.
2. More [advanced](https://adv-r.hadley.nz/fp.html) R discussions refer to the fact that functions are first-class objects in R, meaning that you can look at their properties or do operations on them, just like you can do to other data structures.
...
By the numbers, most R users are probably more familiar with the meaning of [1] than [2],
...
They may also realize that underneath R's interfaces are a lot of functional ideas, especially the tidyverse's "tidy evaluation".
...
Our path from beginning to end requires us to leave `x` in some intermediate state at many different steps, and the present state of `x` can causes a lot of problems for us as we try to plot our way to our destination.
...
1. Function composition creates a _new function_.
   ...
   In the example, `a` is the composition, which is only evaluated on data when it is necessary.
...
This function should a data frame into an iterable collection of groups (like a list), apply `means` to each element of that collection, and return a final data frame that re-merges the collection.
